...
import com.sharuns.popularmovies.BuildConfig;
import com.sharuns.popularmovies.R;
import com.sharuns.popularmovies.Utility;
...
/**
 * Created by sharuns on 4/29/2017.
 */

public class MovieLoader {
    public final String LOG_TAG = MovieLoader.class.getSimpleName();
    private static MovieLoader instance = null;
    ...
    public void loadData(Context context) {
        ...
        if(mSortByParamValue.equals(context.getString(R.string.pref_sort_order_favorite))) {
            ...
            mMovieIds = loadMovies();

            if (mMovieIds == null || mMovieIds.length == 0){
                return;
            }

            //Utility.setMovieIDs(mMovieIds);
            FetchMovieTrailerTask trailersTask = new FetchMovieTrailerTask();
    ...
    private String[] getMoviesDataFromJson(String moviesJsonStr)
        ...
        try{
            ...
            Log.d(TAG, "Movie Initial Data Loading Task Complete. " + inserted + " rows inserted");
    ...
    public class FetchMovieTrailerTask extends AsyncTask<Void, Void, Void> {
        @Override
        ...
        protected void onPostExecute(Void params) {
            FetchMovieReviewsTask reviewsDataTask = new FetchMovieReviewsTask();
            reviewsDataTask.execute();
        }
    }

    public class FetchMovieReviewsTask extends AsyncTask<Void, Void, Void> {
        @Override
        protected Void doInBackground(Void[] params) {
    ...
    private void deleteOldData() {
        ...
        if (favoriteMovieIds != null && favoriteMovieIds.length > 0) {
            String favoriteMovieIdsString = Utility.arrayToString(favoriteMovieIds);
